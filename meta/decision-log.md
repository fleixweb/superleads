...
## 2026-07-28：Code Slice AC 落地资料时效 / freshness 降级

- 决策：产品出海市场分析新增 `freshness_records`，对法规、关税、认证/准入、COO、出口要求、线上价格、Google Trends、物流、近期外部因素、市场报告和季节窗口做时效降级。
- 原因：外贸交付中最危险的弱证据错误之一，是把旧税表、旧法规、无日期网页、只被观察过的消息写成“最新 / 现行 / current / latest”；用户需要看到资料新旧、复核建议和不能当什么。
- 后果：导出层新增 `资料时效`、`复核建议`、`不能当最新结论`；validator 阻断 observed_at 冒充来源日期、过期资料称 current、强时效行误标 `not_time_sensitive`、旧资料/日期未见资料保持 verified 或写最新。首版默认复核窗口已冻结，后续 Authority registry 可按国家、机构和来源类型覆盖。

## 2026-07-28：Slice AD 冻结开放世界权威来源识别模型

- 决策：把后续 Authority registry 方向纠偏为开放世界权威来源识别模型，而不是按国家/地区逐一预置官方来源清单。
- 原因：Superleads 面向全球外贸任务，用户的出口申报国、原产国、起运国、目标销售国和事实域无法穷举；来源是否权威必须由来源身份、事实域、管辖范围、可见证据和时效共同判断。
- 后果：少量国家/地区只能作为 fixture 或加速样例，不作为覆盖边界；未预置国家也应生成动态权威来源发现查询计划；无法核验来源身份或事实域能力时必须降级，不能输出确定性法规、关税、认证、COO 或出口要求结论。


## 2026-07-29：Slice AE 冻结用户可见状态映射

- 决策：Superleads 用户可见交付层采用 11 个统一人话状态词，并把“业务/规则结论”“依据状态”“用户材料状态”分开展示。
- 原因：前序弱证据、互证、时效和权威来源机制让内部枚举变多；如果直接暴露给外贸用户，会变成机器状态机，降低判断效率并诱发状态误用。
- 后果：内部 schema 枚举暂不删除；Markdown / CSV / Excel 导出后续应通过状态投影层显示“已有明确依据、按已知数据计算、多来源方向一致、可作为线索、需补充资料、需权威/专业复核、资料过旧需复核、来源受限、说法冲突待复核、本轮未执行、暂不适用”等人话状态。COO、认证/准入、用户材料状态必须分列，不能互相推导。


## 2026-07-29：Code Slice AE 落地用户可见状态投影

- 决策：新增统一状态投影层，并让产品出海市场分析导出使用 `依据状态` / `依据说明` 等人话字段，用户可见状态收口为 Slice AE 的 11 个状态词。
...
- 决策：将 Superleads 首句路由从简单关键词命中进一步拆成三类意图：客户开发、单一客户背调、产品出海市场分析，并为混合任务输出 `secondary_routes` / `route_order`。
- 原因：真实外贸话术里，“开发市场”“认证需求的进口商”“客户问我要 SDS”“查一下这个网站靠不靠谱”容易因同词误判；入口一旦走错，后面的 schema、validator 和交付器都会服务错路线。
- 后果：`开发某地市场` 和 `找需要 CE/UL 认证的进口商` 仍走批量客户开发；`出口某国是否需要 CE/UL/SDS/UN38.3/COO、关税、标签、清关文件` 走产品出海市场分析；指定域名/官网/邮箱/LinkedIn 并问靠谱、真买家或背后公司时走单一客户背调。路由器仍只是 guardrail，不做搜索、不生成事实、不自动执行第二阶段。

## 2026-07-30：Slice AH 纠偏为发现候选池分区并堵初筛绕过口
