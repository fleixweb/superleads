...
- `python3 evals/run_evals.py --suite default`：`76/76`
- `python3 evals/run_evals.py --suite deep`：`622/622`
- `python3 evals/run_evals.py --suite all`：`662/662`
- `source.open` 已通过 `https://example.com` 公开 GET 烟测恢复

结论更新：规则、图谱校验、审计与导出链路稳定；`source.open` 已恢复，但 `search.web` 仍未知。下一步优先争取搜索能力，或改用用户提供的公开 URL/目录材料继续执行。
