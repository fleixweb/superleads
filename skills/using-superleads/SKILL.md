...
6. For a real-business UAT, initialize `scripts/measure_superleads_uat.py` in a dedicated `/tmp` run directory before research. Record every actual preflight, input precheck, validator, audit (when applicable), Markdown export, workbook export, user-visible, and claimed-path gate. Record failed attempts with the prescribed failure class, stop active-time intervals around waits, and finish with `finalize`. Never describe a corrected run as first-pass success, never compare Git status through hand-written text, and never estimate token usage when the host did not expose it.
