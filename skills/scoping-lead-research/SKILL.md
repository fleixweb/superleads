...
- For `customer_background_research`, collect the user's original request and at least one legal anchor in `background_research_target`. Do not require product, country, ICP, scope axis, or a pre-resolved Entity. Retain Candidate ID, brand, domain, address, email, and user-material literals as supplied. The subject may be `unresolved`, `resolved`, or `multiple_candidates`; do not force a brand site, parent, operating company, factory, or importer into one Entity without evidence.
...
- Use user-facing output modes: 发现候选池, 标准开发名单, 联系方式优先, 补全已有表格, 客户背调报告. `客户背调报告` currently means a research-draft data contract only; this local deployment does not provide `full_review_package`. `初筛客户名单` is not a separate safe delivery mode for default discovery; default weak-evidence delivery must stay in `发现候选池` and use internal `分区` / `依据状态` to express intermediate states.
