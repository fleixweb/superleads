...
用户只输入 `@`、`@superleads`，或询问帮助、怎么用、你能干嘛等简短使用方法时，直接用用户语言返回 `using-superleads` 的对应内联静态引导。不运行工具：不运行 shell、不搜索、不做能力预检，也不检查版本、创建研究对象或加载本 Playbook 其余内容。明确要求详细用法时，返回其中的详细内联引导。
...
元数据、资料初审和帮助不开始本路线研究。只向用户显示业务语言，不显示 JSON、内部阶段名、路径、预检或适配器机制、脚本自我纠偏、解释器、依赖、运行时安装、工作区目录、PYTHONPATH 或模块细节；这同样适用于阶段性进度旁白。
...
默认交付给用户的是业务人员能直接读懂、能据此核对的 Markdown 表格，不使用 `Claim`、`Hypothesis`、`EntityRelationship`、`Observation`、`bridge candidate` 等内部术语作为表头或正文。先给一句不超过两句的白话事实说明，再按以下六张固定表输出；只有存在 `suspected_trade_records` 时才追加第七张条件表，不输出空的贸易记录表：

1. **客户一眼看懂**：这是谁、公开在做什么、是否具备继续核验基础、有哪些公开联系入口、下一步待确认什么。
2. **客户、品牌与关联方**：名称、它是什么、和客户的关系、目前把握、我们依据什么。
3. **公开业务信号与待核验事项**：公开看到的信号、公开关联依据、待核验事项、状态；不提供切入策略或行动建议。
4. **公开联系入口与关联依据**：公开联系入口或职业线索、与主体的公开关联依据、待核验事项、状态；不指定优先联系人或沟通问题。
5. **待核验事项与来源限制**：待核验事项、公开依据或来源限制、状态、处理边界；不把待核验项写成行动建议。
...
脚本不可用时，按 `../../shared/references/no-script-delivery-contract.md` 的唯一契约在对话中按上述六张表交付，并在存在贸易摘要时追加第七张条件表；必须标注“本环境未运行确定性校验”，且不得声称已生成 Markdown、CSV 或 XLSX 文件。无脚本交付仍须逐项遵守同一主体、已打开来源、联系方式归属、未知与冲突保留等边界。
...
- “是否具备继续核验基础”只汇总当前可核实的主体、业务和公开联系信息；不判断客户价值、采购意愿、是否应跟进、报价或谈判结论。
