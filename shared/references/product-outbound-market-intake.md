...
只有明确要求完整/整体市场分析时，才覆盖趋势、公开价格参考、行业资料、准入合规、进口税费、出口要求、运输路线、季节窗口和近期外部因素，并整理成完整十二张表；认证会先查目标市场可能要求什么，再单独看你有没有对应材料；不输出是否值得进入，也不生成客户名单。
如果你只问其中一项或几项，我会只做请求的模块和三张固定表，并在报告开头明确本轮范围和本轮未执行模块。只说“分析某市场”但未说明维度时，先用一句短问题确认范围，不擅自扩成完整报告。
```
...
- 明确的“完整市场分析”“整体市场分析”“全面分析” -> 完整报告（全部模块）。字段缺失、空数组、无法识别或用户意图不确定时，先确认范围或交付明确标注的最小研究快照，不能自动变成完整报告。
- 明确点名一项或多项模块时，只写入这些模块，不扩成完整报告：认证、测试、注册、标签、SDS、UN38.3、CE、UL、SABER -> `certification`；关税、税率、HS/HTS、税费 -> `import_tax`；COO / 原产地证明 -> `certification`（包含 origin-proof 行）；清关、物流、运输、预申报 -> `logistics`；出口报关、商检、出口管制 -> `export_requirements`；趋势、价格、好不好卖 -> `google_trends` + `online_price` + `market_reports`。
- 例如“只做美国准入、税费、出口文件和物流” -> `destination_compliance`、`import_tax`、`export_requirements`、`logistics`。这是四模块范围，不是完整十二表。

完整报告输出全部十二张表。显式范围报告只输出所选模块对应的表，加上固定的
`市场事实总览`、`产品档案与触发项`、`信息来源与待确认事项`，并在开头清楚列出
未请求的模块为“本轮未执行”。显式范围报告开头使用以下范围声明模板：
...
请求多个模块时，列出所有已选范围组，例如：

```text
本轮范围：只做「目标国准入与认证要求、进口税费、出口国要求、物流与申报」。
本轮未执行：{未请求模块}。
```
...
- Product name, category, use description, product URL/PDF, image clue, HS/HTS code, model, part number, OEM number, 番号 or 料号 can start first-pass category-level analysis. For a bare code, first use 公开检索 to identify the product and its public manufacturer/brand association; this is source collection, 不是模型推断. Until an opened source supports the identity, keep it as a product clue rather than a confirmed category. A missing model/version lowers precision; it does not force the assistant to stop at a pending-material checklist.
