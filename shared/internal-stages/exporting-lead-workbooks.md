...
`export_superleads_markdown.py --route bulk_customer_development` and should be
recognized by `# 批量客户开发`, `发现候选池样表（候选池不是正式开发名单）`, and
the main table columns `分区`, `候选客户`, `业务相关性`, `依据状态`, `来源 /
来源状态`. The raw workbook sheet header `公司名称 | 国家/地区 | 官网/域名`
is not a valid chat-facing bulk Markdown main table.
